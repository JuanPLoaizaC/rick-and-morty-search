      ...
      <aside className="fixed top-0 left-0 z-40 w-1/3 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          ...
          <div className="flex ml-12">
            <p className="text-sm leading-6 text-gray-600">STARRED CHARACTERS ({charactersList.filter(character => character.favourite).length})</p>
          </div>
          <ul role="list" className="divide-y divide-gray-100 mb-5">
            {charactersList.filter(character => character.favourite).sort((a, b) => {
              if (sortOrder === 'asc') {
          ...
          <div className="flex ml-12">
            <p className="text-sm leading-6 text-gray-600">CHARACTERS ({charactersList.filter(character => !character.favourite).length})</p>
          </div>
