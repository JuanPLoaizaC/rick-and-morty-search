        ...
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          ...
          <div className="relative mt-2.5">
            ...
            <div className="absolute inset-y-0 right-0 flex items-center">
              <Popover className="relative">
                <Popover.Button className='hover:bg-gray-100 rounded-lg'>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 text-purple-700">
                    ...
                    <div>
                      <div className="flex flex-col">
                        <div className="flex">
                          ...
                          <button className="hover:bg-purple-100 font-semi-bold py-2 px-4 rounded-lg border border-gray-300" onClick={() => {
                            setFilterButtons({
                              ...filterButtons,
                              character: 'all'
                            })
                          ...
                          <button className="hover:bg-purple-100 font-semi-bold py-2 px-4 rounded-lg border border-gray-300 ml-2" onClick={() => {
                            setFilterButtons({
                              ...filterButtons,
                              character: 'starred'
                            })
                        ...
                        <div className="flex mt-4">
                          <div className="mr-5">
                            <label for="specie-filter" className="text-white">Specie</label>
                          </div>
                          <button className="hover:bg-purple-100 font-semi-bold py-2 px-4 rounded-lg border border-gray-300" onClick={() => {
                            setFilterButtons({
                              ...filterButtons,
                      ...
                      <div className="flex justify-center">
                        <button className="bg-purple-700 text-white font-bold py-2 px-4 rounded-lg mt-5 mb-5" style={{ width: '85%' }} onClick={() => setFilterFlag(true)} >
                          Filter
                        </button>
                      </div>
                    </div>
            ...
            filterButtons.character !== 'others' &&
              <>
                <div className="flex ml-12 mt-8">
                  <p className="text-sm leading-6 text-gray-600">STARRED CHARACTERS ({charactersList.filter(character => {
                    return character.favourite && (character.species.toUpperCase() === filterButtons.specie.toUpperCase() || filterButtons.specie === 'all') && character.name.toUpperCase().includes(filter.toUpperCase());
                  }).length})</p>
                </div>
          ...
          }
          {
            filterButtons.character !== 'starred' &&
              <>
                <div className={`flex ml-12 ${filterButtons.character === 'others' ? 'mt-8' : ''}`}>
                  <p className="text-sm leading-6 text-gray-600">CHARACTERS ({charactersList.filter(character => {
                    return !character.favourite && (character.species.toUpperCase() === filterButtons.specie.toUpperCase() || filterButtons.specie === 'all') && character.name.toUpperCase().includes(filter.toUpperCase());
                  }).length})</p>
                </div>
